...
<form method="POST" action="{{ $article->exists ? route('admin.articles.update', $article) : route('admin.articles.store') }}" class="space-y-6">
    ...
    <div class="rounded-xl bg-white border border-slate-200 p-5 sm:p-6 space-y-5 shadow-sm">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Nama Penulis Artikel</label>
                <input
                    name="author_name"
                    value="{{ old('author_name', $article->author_name) }}"
                    class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#3FA7D6]/40"
                    placeholder="Ragil atau Tim Redaksi Logistax"
                >
                <p class="mt-1 text-xs text-slate-500">Nama ini akan tampil di halaman publik sebagai penulis artikel. Kosongkan jika ingin memakai nama akun author.</p>
            </div>
