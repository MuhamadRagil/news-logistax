...
<form method="POST" action="{{ route('admin.pages.update', $page) }}" class="space-y-6">
    ...
    <div class="bg-white border border-slate-200 p-6 space-y-4">
        <div>
            ...
            <input name="title" value="{{ old('title', $page->title) }}" class="w-full border border-slate-300 px-3 py-2">
        </div>
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Body</label>
            <textarea name="body" rows="14" class="w-full border border-slate-300 px-3 py-2">{{ old('body', $page->body) }}</textarea>
        </div>
        <div class="grid sm:grid-cols-3 gap-4">
            ...
            <input name="meta_title" value="{{ old('meta_title', $page->meta_title) }}" class="border border-slate-300 px-3 py-2" placeholder="Meta title">
            <input name="meta_description" value="{{ old('meta_description', $page->meta_description) }}" class="border border-slate-300 px-3 py-2" placeholder="Meta description">
        </div>
    </div>
