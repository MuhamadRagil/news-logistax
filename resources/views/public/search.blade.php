...
<section class="border-b border-stone-200 pb-6">
    ...
    <form method="GET" class="mt-4 max-w-2xl flex gap-2">
        <input
            name="q"
            value="{{ $q }}"
            class="flex-1 border border-stone-300 bg-white px-3 py-2"
...
<div class="mt-8 space-y-4">
    @forelse($articles as $article)
        <article class="bg-white border border-stone-200 p-5">
            <a
                ...
                class="text-xl font-semibold text-slate-900 hover:text-slate-700"
            >
                {{ $article->title }}
            </a>
    ...
    @empty
        <div class="bg-white border border-stone-200 p-8 text-center text-slate-500">
            Tidak ada hasil ditemukan untuk kata kunci "{{ $q }}".
        </div>
    @endforelse
</div>
