...
<div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 border-b border-stone-200 pb-6">
    ...
    <form method="GET" class="grid sm:grid-cols-3 gap-2 text-sm w-full md:w-auto">
        <input
            name="q"
            value="{{ request('q') }}"
            class="border border-stone-300 bg-white px-3 py-2"
        ...
        <button class="px-4 py-2 bg-slate-900 text-white font-medium">
            Terapkan
        </button>
    </form>
</div>
    ...
    @forelse($articles as $article)
        <article class="bg-white border border-stone-200 p-6">
            ...
            <a
                ...
                class="block mt-2 text-2xl font-semibold leading-tight text-slate-900 hover:text-slate-700"
            >
                {{ $article->title }}
            </a>
