...
@section('content')
<form method="POST" action="{{ $category->exists ? route('admin.categories.update', $category) : route('admin.categories.store') }}" class="max-w-2xl rounded-xl bg-white border border-slate-200 p-6 space-y-4 shadow-sm">
    @csrf
    @if($category->exists)
        @method('PUT')
    @endif
    ...
    <div class="flex gap-3 pt-2">
        ...
        <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 rounded-lg border border-slate-300 bg-white font-semibold hover:bg-slate-50 transition-colors">Back</a>
    </div>
</form>
@endsection