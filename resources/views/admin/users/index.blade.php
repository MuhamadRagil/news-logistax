...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <a href="{{ route('admin.users.create') }}" class="inline-flex items-center justify-center rounded-lg bg-[#0F4C6C] px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-[#0d425d]">
        Add User
    </a>
</div>
        ...
        <table class="w-full min-w-[900px] text-sm">
            <thead class="bg-slate-50 text-slate-600">
                <tr>
                    ...
                    <th class="px-5 py-3 text-left font-bold">Last Login</th>
                ...
                @forelse($users as $user)
                    <tr class="border-t border-slate-200 hover:bg-slate-50/70">
                        <td class="px-5 py-4">
                            ...
                            <div class="text-xs text-slate-500 font-mono">ID: {{ $user->id }}</div>
                        </td>
                        <td class="px-5 py-4 text-slate-600">{{ $user->email }}</td>
                        <td class="px-5 py-4">
                            <div class="flex flex-wrap gap-2">
                                @forelse($user->roles as $role)
                                    <span class="rounded-full bg-[#E6F4FA] px-2.5 py-1 text-xs font-semibold text-[#0F4C6C]">
                                        {{ $role->name }}
                                    </span>
                                @empty
                                    <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-500">No role</span>
                                @endforelse
                            </div>
                        </td>
                        <td class="px-5 py-4">
                            @if($hasIsActiveColumn)
                                ...
                                @else
                                    <span class="rounded-full bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700 ring-1 ring-rose-200">Inactive</span>
                                @endif
                            @else
                                <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-500">N/A</span>
                            @endif
                        </td>
                        <td class="px-5 py-4 text-slate-600 font-mono">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex flex-wrap items-center justify-end gap-3 font-semibold">
                                <a href="{{ route('admin.users.edit', $user) }}" class="text-blue-700 hover:text-blue-900">Edit</a>
                                <a href="{{ route('admin.users.edit', $user) }}#role-card" class="text-[#0F4C6C] hover:text-[#0d425d]">Edit Role</a>
